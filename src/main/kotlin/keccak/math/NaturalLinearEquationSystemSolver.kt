...
fun solveNaturalLinearEquationsSystem(
    a: Array<LongArray>,
    b: LongArray,
    ...
    fun substitute(i: Int, j: Int, k: Int) {
        ...
        var c = colsMask?.nextSetBitDefault(0, cols) ?: 0
        while (c < cols) {
            a[j][c] = sign * jMult.toLong() * a[i][c] + iMult.toLong() * a[j][c]
            c = colsMask?.nextSetBitDefault(c + 1, cols) ?: (c + 1)
        }

        b[j] = sign * jMult.toLong() * b[i] + iMult.toLong() * b[j]
    }
    ...
    row = rowsMask?.nextSetBitDefault(0, rows) ?: 0

    while (row < rows) {
        val gcdValue = gcd(gcd(a[row]), b[row].toULong()).toLong()

        if (gcdValue != 1L) {
            col = colsMask?.nextSetBitDefault(0, cols) ?: 0

            while (col < cols) {
                a[row][col] = a[row][col] / gcdValue
                col = colsMask?.nextSetBitDefault(col + 1, cols) ?: (col + 1)
            }

            b[row] = b[row] / gcdValue
        }
